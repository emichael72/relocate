...
Description:
    Generic logging module allows for console and file logging, with and without ANSI colors.
"""
...
class LoggerColorFormatter(logging.Formatter):
    ...
    def _logger_message_format(message: str):
        ...
        """
        error_parts = []  # Build the error message parts and clean each component
...
def logger_setup(name='Undefined', level=logging.DEBUG, log_file=None, no_colors=False):
    """
    ...
    if not local_logger.hasHandlers():
        ...
        formatter = LoggerColorFormatter(log_format, datefmt=base_date_format, no_colors=no_colors)
        ...
        if log_file is not None:
            file_formatter = LoggerColorFormatter(log_format, datefmt=base_date_format, no_colors=True)
        ...
        local_logger.addHandler(logs_counter)
        local_logger.name = name
        return local_logger
